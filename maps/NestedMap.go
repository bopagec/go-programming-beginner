...
It takes a slice of strings (names) and returns a nested map where the first key is all the unique first characters of the names, the second key is all the names themselves, and the value is the count of each name.
...
*/
func getNameCounts(names []string) map[rune]map[string]int { // runes represent individual characters rather than a string
...
func test5(names []string, initial rune, name string) {
...
func main() {
	test5(getNames(50), 'M', "Matthew")
	test5(getNames(100), 'G', "George")
	test5(getNames(150), 'D', "Drew")
	test5(getNames(200), 'P', "Philip")
	test5(getNames(250), 'B', "Bryant")
	test5(getNames(300), 'M', "Matthew")
}
