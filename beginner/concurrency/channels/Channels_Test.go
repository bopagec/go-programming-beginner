...
The filterOldEmails function is trying to send on a channel, but no other goroutines are running that can accept the value from the channel.
...
func filterOldEmails(emails []email) {
	...
	// and this will happen at the same time in go routines
	isOld := <-isOldChan
